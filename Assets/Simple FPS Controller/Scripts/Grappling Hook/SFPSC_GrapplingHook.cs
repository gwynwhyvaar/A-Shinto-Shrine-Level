...
public class SFPSC_GrapplingHook : MonoBehaviour
{
    ...
    private IEnumerator Decelerate()
    {
        ...
        max = deceleratingTime * Mathf.Clamp01(targetDistance / 10.0f) * Mathf.Clamp01(rb.linearVelocity.magnitude / 30.0f);
        for (; decelerateTimer < max; decelerateTimer += Time.deltaTime)
        {
            rb.AddForce(-rb.linearVelocity.normalized * deceleration * (1.0f - decelerateTimer / max) * Mathf.Clamp01(rb.linearVelocity.sqrMagnitude / 400.0f) * Time.deltaTime, ForceMode.Acceleration);
            yield return wfeof;
        }
        decelerateTimer = 0.0f;
    }
    ...
    private void GrappleUpdate()
    {
        ...
        rb.linearVelocity = Vector3.Lerp(rb.linearVelocity, dir * maximumSpeed * Mathf.Clamp01(targetDistance / (4.0f * distanceToStop)), Time.deltaTime);
